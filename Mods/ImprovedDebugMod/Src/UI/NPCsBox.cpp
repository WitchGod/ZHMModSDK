...
void ImprovedDebugMod::DrawNPCsBox(bool p_HasFocus)
{
    ...
    if (s_Showing && p_HasFocus)
    {
        ...
        if (ImGui::Button("Teleport NPC To Player"))
            TeleportActorToHitman(s_Actor);
        ImGui::SameLine();

        if (ImGui::Button("Teleport Player to NPC"))
            TeleportHitmanToActor(s_Actor);
        ImGui::SameLine();

        if (ImGui::Button("Swap Positions with NPC"))
            SwapHitmanAndActor(s_Actor);
